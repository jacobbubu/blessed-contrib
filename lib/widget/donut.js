...
Donut.prototype.update = function(data) {
  ...
  var percentMark = this.options.percent;

  var middle = cheight / 2;
  var spacing = (cwidth - donuts * radius * 2) / (donuts + 1);

  function drawDonut(label, percent, value, radius, width, cxx, middle, color) {
    makeRound(100, radius, width, cxx, middle, remainColor);
    makeRound(percent, radius, width, cxx, middle, color);
    var ptext =
      (value ? value : parseFloat(percent * 100).toFixed(0)) + percentMark;
    c.fillText(
      ptext,
  ...
  function makeDonut(stat, which) {
    ...
    var cxx = left;
    drawDonut(label, percent, value, radius, width, cxx, middle, color);
  }
